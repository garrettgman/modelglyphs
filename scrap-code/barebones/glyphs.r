...
glyphs <- function(data, x_minor, y_minor, x_major = NULL, y_major = NULL, polar = FALSE, height = rel(0.95), width = rel(0.95), y_scale = identity, x_scale = identity, quiet = FALSE) {
  ...
  if (polar) {
    ...
    data$.x <- data[[x_major]] + width  / 2 * r * sin(theta) 
    data$.y <- data[[y_major]] + height / 2 * r * cos(theta)
    data <- data[order(data[[x_major]], data[[x_minor]]), ] 
  } else {
    data$.x <- data[[x_major]] + rescale11(data[[x_minor]]) * width / 2
    data$.y <- data[[y_major]] + rescale11(data[[y_minor]]) * height / 2 
  }
  
  structure(data, 
    width = width, height = height, polar = polar, 
    x.major = x_major, y.major = y_major,
